...
def cmd_init(
    force: bool = typer.Option(
        False, "--force", "-f", help="Overwrite chaos.yaml if it exists."
    )
) -> None:
    """Auto-discover docker-compose.yml and generate a chaos.yaml file."""
    import yaml
    
    out_file = Path("chaos.yaml")
    if out_file.exists() and not force:
        _err_panel(f"'{out_file}' already exists. Use --force to overwrite.", "File Exists")
        raise typer.Exit(1)
        
    compose_files = [Path("docker-compose.yml"), Path("docker-compose.yaml"), Path("compose.yaml")]
    found = None
    ...
    console.print(f"\n  [bold green]✓[/bold green] Found [cyan]{found.name}[/cyan]")
    
    try:
        with open(found, "r") as f:
            data = yaml.safe_load(f)
    except Exception as e:
        _err_panel(f"Failed to parse {found.name}: {e}", "Parse Error")
        raise typer.Exit(1)
        
    services = data.get("services", {})
    if not services:
        _err_panel(f"No services found in {found.name}.", "No Services")
        raise typer.Exit(1)
        
    service_names = list(services.keys())
    console.print(f"  [bold green]✓[/bold green] Discovered [bold]{len(service_names)}[/bold] services: {', '.join(service_names)}")
    ...
    chaos_config = {
        ...
        "actions": [
            "stop",
            "restart",
            {"name": "delay", "latency_ms": 300, "duration": 15},
            {"name": "limit_cpu", "cpus": 0.2, "duration": 15}
        ],
        "safety": {
            "max_down": 1,
            "cooldown": 20,
